...
<style type="text/css">
  ...
  .img{
    ...
    <div class="card-body">
      @if(isset($order))
      <div class="row p-2">
       <h6>ORDER-ID: <strong><small>{{$order->order_id}}</small></strong></h6> 
      </div>
      @endif
      <div class="row p-2">
...
const getItemBox = async () =>{
 ...
 timeOutCall =  setTimeout( ()=>{
  loadoverlay($("#item_box_row"))
   $.get("{{route('get-item-box')}}",{type:"product",sub_category : $("#sub_category_easy").val(),order_id:$("#order_id").val(),search:$("#search_key_item_box").val()})
    .then((data)=>{
